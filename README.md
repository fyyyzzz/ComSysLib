...
> ⚠️ **注意**
>
> 2.4 和2.5 索引部分比较耗时，因此已经在 `rageng` 下完成了一次索引（即完成了GraphRAG前期实体关系提取、社区挖掘、摘要生成等任务，索引的文档是《A Christmas Carol》）。  
> 测试或演示时可以直接跳到 2.6 进行查询。  
> 
> 如果想检索或索引新的文档，请按照 2.4/2.5 的流程，创建新的项目文件夹（如 `ragtest`）进行索引。
...
- **ragtest/output/** ：检索、索引等过程的参数配置，后面主要在这里修改索引、检索设置。该目录包含以下文件：
...
- **ragtest/cache/** ：索引过程中的缓存
- **ragtest/logs/** ：索引过程中的日志
